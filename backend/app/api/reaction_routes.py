...
@reaction_routes.route("/<int:message_id>/reaction/<int:id>", methods=["DELETE"])
@login_required
def remove_emoji(message_id, id):
    data = request.get_json()
    channel_filter = Channel_Message_Reaction.query.filter_by(
    ...
    reaction_to_delete = Channel_Message_Reaction.query.get(id)
